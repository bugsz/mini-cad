...
{ 
	...
	case WM_KEYDOWN:
		if (g_keyboard != NULL)
			g_keyboard(wParam==VK_PROCESSKEY?ImmGetVirtualKey(g_hWnd):(int) wParam,KEY_DOWN);
		break;
...
void updateFont()
{
	...
	g_font = CreateFontA(
		g_textSize,
		0,
		0,0,400,0,0,0,0,0,0,0,0,g_fontName);
...
/////////////////////////////////////////////////////
void registerRender(RenderCallback callback){
	g_render = callback;
}
void setReDraw(){
	isNeedToDraw = 1;
